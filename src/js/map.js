...
function perturbVertex( vertex ) {
  return vertex.multiplyScalar( _Math.randFloat( 1, 1.5 ) );
}
...
const minimal = {
  pointLight: (() => {
    const light = new PointLight( '#aaf' );
    light.position.set( 8, 8, 8 );
    return light;
  })(),

  hemisphereLight: new HemisphereLight( '#d43', '#33a', 0.5 ),
};
...
const maps = {
  ...
  artifacts( scene ) {
    const baseGeometry = new BoxGeometry( 16, 1, 1 )
      .translate( 0, 5, 0 );

    baseGeometry.merge( new BoxGeometry( 12, 1, 1 ) );

    const artifactGeometry = greeble( baseGeometry, {
      count: 100,
      greeble() {
        return new BoxGeometry(
          _Math.randFloat( 0.5, 2 ),
          _Math.randFloat( 0.5, 2 ),
          _Math.randFloat( 0.5, 2 )
        );
      },
    ...
    const artifact = new Mesh( artifactGeometry, new MeshPhongMaterial({
      shading: FlatShading,
      color: '#333',
      shininess: 10,
    }));
  ...
  minimalSkybox( scene ) {
    ...
    const asteroidGeometry = new IcosahedronGeometry( 1, 1 );
    ...
    let count = 32;
    while ( count-- ) {
      const geometry = new Geometry().copy( asteroidGeometry );
      geometry.vertices.forEach( perturbVertex );

      const angle = 2 * Math.PI * Math.random();

      position.set(
        radius * Math.cos( angle ) + _Math.randFloatSpread( positionSpread ),
        radius * Math.sin( angle ) + _Math.randFloatSpread( positionSpread ),
        _Math.randFloatSpread( positionSpread )
      );
      ...
      scale.set(
        1 + _Math.randFloatSpread( scaleSpread ),
        1 + _Math.randFloatSpread( scaleSpread ),
        1 + _Math.randFloatSpread( scaleSpread )
      );
      ...
      asteroidBelt.add( new Mesh( geometry, material ) );
    }
    ...
    const nebulaGeometry = new Nebula(
      new IcosahedronGeometry( 64, 3 ),
      {
        period: 64,
        fromColor: new Color( '#223' ),
        toColor: new Color( '#000' ),
      }
    );

    const nebulaMesh = new Mesh(
      nebulaGeometry,
